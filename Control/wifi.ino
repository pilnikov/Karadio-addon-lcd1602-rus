bool sta_init()
{
  ...
  if (!result)
  {
    wifi_conn(1, 0); //сообщение на индикатор
    WiFi.disconnect();
    WiFi.mode( WIFI_STA );
...
//-------------------------------------------------------------- Установка соединения с точкой доступа WIFi
{
  ...
  if (!result)
  {
    ...
    while (j < 6 && !result)
    {
      ...
      wifi_conn(3, WiFi.status()); //сообщение на индикатор
      if (!result)
      {
        delay(2000);
        j++;
    ...
    if (result)
    {
      ...
      wifi_conn(4, 0); //выдать радостную новость на индикатор
      WiFi.setAutoConnect(true);
      WiFi.setAutoReconnect(true);
      serv_ms = millis();
    }
    else wifi_conn(6, 0); //выдать нерадостную новость на индикатор
  }
  return result;
}
...
bool ap_init()
{
  ...
  if (!result)
  {
    wifi_conn(2, 0); //сообщение на индикатор
    ...
    if  (WiFi.getMode() == WIFI_AP)
    {
      ...
      wifi_conn(5, 0); //выдать радостную новость на индикатор
      serv_ms = millis();
    }
    else wifi_conn(6, 0); //выдать нерадостную новость на индикатор
